...
class SchedulingService {
     ...
     */
    static mergeBusyIntervals(intervals) {
     ...
     */
    static async getInterviewerBusySlots(interviewers, timeMin, timeMax) {
        const allBusy = [];
        for (const interviewer of interviewers) {
            // 1. Google Calendar busy slots if connected
            if (interviewer.googleCalendar?.connected) {
                try {
                    const userWithTokens = await User_1.User.findById(interviewer._id).select('+googleCalendar.accessToken +googleCalendar.refreshToken');
                    ...
                    const googleBusy = await googleCalendar_1.GoogleCalendarService.getFreeBusy(accessToken, refreshToken, interviewer.googleCalendar.calendarId || 'primary', timeMin, timeMax);
            ...
            const existingInterviews = await Interview_1.Interview.find({
     ...
     */
    static async generateAvailableSlots(options) {
        ...
        const searchStart = (0, date_fns_1.isAfter)(minNoticeDate, windowStart) ? minNoticeDate : windowStart;
        const searchEnd = windowEnd;
        if ((0, date_fns_1.isAfter)(searchStart, searchEnd)) {
            return [];
        }
        // Load interviewers
        const interviewers = await User_1.User.find({ _id: { $in: interview.interviewerIds } });
        ...
        const candidateCustomAvail = await Availability_1.Availability.find({ interviewId: interview._id });
        ...
        if (remainder !== 0) {
            currentCursor = (0, date_fns_1.addMinutes)(currentCursor, slotIntervalMinutes - remainder);
        }
        ...
        while (currentCursor.getTime() + durationMs <= searchEnd.getTime()) {
            ...
            const slotStartInInterviewTz = (0, date_fns_tz_1.toZonedTime)(slotStart, interview.timezone);
            const slotEndInInterviewTz = (0, date_fns_tz_1.toZonedTime)(slotEnd, interview.timezone);
            ...
            const isWithinWorkingHours = slotStartInInterviewTz.getDate() === slotEndInInterviewTz.getDate() &&
            ...
            if (isWithinWorkingHours) {
                ...
                if (!overlapsWithBusy) {
                    ...
                    if (passesCandidateCustom) {
                        const formattedStart = (0, date_fns_tz_1.format)(slotStart, 'yyyy-MM-dd HH:mm (z)', { timeZone: targetTimezone });
                        const formattedEnd = (0, date_fns_tz_1.format)(slotEnd, 'HH:mm (z)', { timeZone: targetTimezone });
                        availableSlots.push({
     ...
     */
    static async validateSelectedSlot(interview, slotStart, slotEnd) {
        // 1. Check window & min notice
        const now = new Date();
        const minNoticeDate = new Date(now.getTime() + (interview.minimumNotice || 12) * 3600 * 1000);
        if ((0, date_fns_1.isBefore)(slotStart, minNoticeDate)) {
            return { valid: false, reason: 'Selected slot violates minimum notice requirement.' };
        }
        if ((0, date_fns_1.isBefore)(slotStart, interview.schedulingWindow.startDate) || (0, date_fns_1.isAfter)(slotEnd, interview.schedulingWindow.endDate)) {
        ...
        const isBlocked = busySlots.some((busy) => slotStart.getTime() < busy.end.getTime() && slotEnd.getTime() > busy.start.getTime());
        if (isBlocked) {
