...
const candidateLogin = async (req, res, next) => {
    try {
        ...
        const candidate = await Candidate_1.Candidate.findOne({ email: email.toLowerCase() }).select('+passwordHash');
        if (!candidate) {
        ...
        // 3. Passwordless email access if candidate has active interviews
            ...
            const activeInterviews = await Interview_1.Interview.countDocuments({ candidateId: candidate._id });
...
const getMe = async (req, res, next) => {
    try {
        ...
        if (req.user.role === 'CANDIDATE') {
            const candidate = await Candidate_1.Candidate.findById(req.user.id);
            if (!candidate) {
